...
char *listDirectory(const char *path)
{
    ...
    while ((entry = readdir(dir)) != NULL)
    {
        ...
        size_t needed = strlen(html) + strlen(entry->d_name) + 128;
        if (needed > size)
        {
            size *= 2;
        ...
        // Remove o prfixo "./root"
        const char *web_path = path;
        if (strncmp(path, "./root", 6) == 0)
        {
            web_path = path + 6;
        }

        snprintf(html + strlen(html), size - strlen(html), "<li><a href=\"%s/%s\">%s</a></li>", web_path, entry->d_name, entry->d_name);
    }
...
{
    size_t len_resp = snprintf(NULL, 0,"HTTP/1.1 %hu %s\r\nContent-Type: %s\r\nContent-Length: %zu\r\n\r\n%s",status, MSG_STATUS, ctn_type, ctn_length, body) + 1;
...
void response(int *client_id)
{
    ...
    {
        // Se for diretório tenta carregar index.html
        char index_path[600];
    ...
    {
        // Arquivo normal podemdo ser texto ou binário
        size_t file_size = st.st_size;
        body = readFiles(route);

        if (body)
        {
            const char *content_type = mimeType(route);
        ...
        else
        {
            ...
            http_response = buildResponseHttp(500, "Internal Server Error", "text/html", strlen(err), err);
            send(client_fd, http_response, strlen(http_response), 0);
        }
    }
    else
